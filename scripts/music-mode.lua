...
-------------------------------------------------------------------------------
PATCHED (local): instead of relying on a separately-configured `undo_profile`,
`deactivate()` now calls `apply-profile <profile> restore`, which uses mpv's
built-in profile-restore mechanism (profile-restore=copy-equal, already set on
the music-mode profile in mpv.conf) to automatically revert exactly the options
that music-mode changed, back to whatever they were immediately before it was
applied. `undo_profile` is no longer used and can be left unset.
-------------------------------------------------------------------------------
...
local o = {
    ...
    --PATCHED: no longer used — deactivate() now uses `apply-profile <profile> restore` instead.
    --Left here only for backwards compatibility with existing configs; safe to leave unset.
    undo_profile = "",
...
local function deactivate()
    --PATCHED: revert whatever `profile` changed via mpv's own profile-restore
    --mechanism (requires profile-restore=copy-equal on the profile itself),
    --instead of applying a separately maintained undo_profile.
    if o.profile ~= "" then mp.commandv('apply-profile', o.profile, 'restore') end
...
local function main()
    ...
    elseif musicMode then
        msg.verbose('video file, restoring profile "' .. o.profile .. '"')
        deactivate()
    end
end
