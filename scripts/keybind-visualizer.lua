...
-- -- see "show_default_bindings" below).
--
...
local BINDING_NAME = "keybind-visualizer"

local options = {
	layout = "abnt2",
	layouts_file = "script-opts/keybind-visualizer-layouts.json",
	-- "auto" (default): ask mpv's own "input-default-bindings" property and
	--   hide mpv's built-in defaults only if that option is confirmed off.
...
-- inverse of parse_combo: rebuild an mpv key-string (e.g. "Ctrl+Shift+q", "Q")
-- from a lowercased base key and modifier flags, so we can register a forced
-- binding that actually matches what the user physically presses.
local function mods_for_bind(base, c, s, a, m)
	local prefix = {}
	if c then
		prefix[#prefix + 1] = "Ctrl"
	end
	if a then
		prefix[#prefix + 1] = "Alt"
	end
	if m then
		prefix[#prefix + 1] = "Meta"
	end
	local key = base
	if s then
		if #prefix == 0 and #base == 1 and base:match("%a") then
			key = base:upper()
		else
	...
	if #prefix > 0 then
		return table.concat(prefix, "+") .. "+" .. key
	end
	return key
end
...
local function defaults_are_shown()
	...
	-- auto: ask mpv directly whether builtin defaults are enabled right now
	local enabled = mp.get_property_native("input-default-bindings")
...
local function build_bindings()
	...
	for _, e in ipairs(list) do
		...
		if section == "default" and (show_all or not e.is_weak) then
...
-- find which physical key (if any) currently triggers this very script, so
-- pressing it again while the overlay is open can close it, same as ESC.
local function find_toggle_key_id()
	local target = "script-binding " .. BINDING_NAME
	for _, k in ipairs(KEYS) do
		local list = bindings_for(k.id)
		if list then
			for _, b in ipairs(list) do
				if b.cmd == target then
					return k.id
				end
			end
		end
...
local function build_capture_keys()
	...
	for _, k in ipairs(KEYS) do
		if k.mpv and k.mpv:upper() ~= "ESC" then
			add(k.mpv, k.id)
			local combos = BK[k.mpv_lc]
			if combos then
				for _, b in ipairs(combos) do
					add(mods_for_bind(k.mpv, b.c, b.s, b.a, b.m), k.id)
				end
			end
		end
	end
...
local function render()
	...
	if hovered then
		...
		if tk then
			close_hint = close_hint .. " / " .. esc(key_name(tk.id, tk.label))
		end
		panel_lines = {
			string.format(
...
mp.add_key_binding(nil, BINDING_NAME, toggle)
